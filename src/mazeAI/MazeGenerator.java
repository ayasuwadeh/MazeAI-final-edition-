	...
	public void hunt() {
		for(int i = 0; i < N; i++) {
			if(cells[i].visited == false) {
				...
				//System.out.println("FOUND" + currentIndex);
	...
	public void drawMaze(Graphics g) {
		...
		for(int i = 0; i < N; i++) {
			...
			if(cells[i].border[NORTH] == WALL) {
				g.drawLine(MARGIN + (CELL_WIDTH * column) , MARGIN + ((row) * CELL_WIDTH), 
						MARGIN + (CELL_WIDTH * (column + 1)), MARGIN + ((row) * CELL_WIDTH));
			}
			if(cells[i].border[EAST] == WALL) {
				g.drawLine(MARGIN + (CELL_WIDTH * (column + 1)), MARGIN + ((row) * CELL_WIDTH),
						MARGIN + (CELL_WIDTH * (column + 1)), MARGIN + ((row + 1) * CELL_WIDTH));
			}
			if(cells[i].border[SOUTH] == WALL) {
				g.drawLine(MARGIN + (CELL_WIDTH * (column + 1)), MARGIN + ((row + 1) * CELL_WIDTH),
						MARGIN + (CELL_WIDTH * column), MARGIN + ((row + 1) * CELL_WIDTH));
			}
			if(cells[i].border[WEST] == WALL) {
				g.drawLine(MARGIN + (CELL_WIDTH * column), MARGIN + ((row + 1) * CELL_WIDTH),
						MARGIN + (CELL_WIDTH * column) , MARGIN + ((row) * CELL_WIDTH));
			}
		}
	}
